    ...
    function updateUI() {
        ...
        if (config.showPossible) gameModifiers.SP = true;
        if (config.showPossible && config.showDetailedPossible) gameModifiers.DP = true;
        if (config.autoShuffle) gameModifiers.AS = true;
        if (config.preventBadShuffle) gameModifiers.PBS = true;
        if (config.autoSelectThird) gameModifiers.A3RD = true;
        if (config.useFixedSeed) gameModifiers.SS = true;
    ...
    function showExtraStats() {
        ...
        const avgPossible = (s.possibleHistory.reduce((a,b) => a+b, 0) / s.possibleHistory.length).toFixed(1);
    ...
    function displayResults(sets, bs, s) {
        ...
        buildModifiersUI(s.modifiers || gameModifiers);
    ...
    function resetStats() {
        ...
        setTimestamps = [];
        possibleHistory = [];
        gameModifiers = { SP: config.showPossible, DP: config.showPossible && config.showDetailedPossible, AS: config.autoShuffle, PBS: config.preventBadShuffle, A3RD: config.autoSelectThird, SS: config.useFixedSeed };
